-- Fix probe count for both live assessment and standalone tests:
-- Resolving probe with 'fail' (Yellow) preserves the probe_count without incrementing.
-- Resolving probe with 'done' (Indigo) increments the probe_count by 1.
...
begin
  ...
  if p_outcome in ('fail', 'done') then
    ...
    set status = 'finalized',
        probe_count = probe_count + case when p_outcome = 'done' and snap.status = 'probe_open' then 1 else 0 end,
