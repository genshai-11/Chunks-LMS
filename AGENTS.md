...
|---|---|---|
| [`modules/catalog`](file:///C:/Users/gensh/Desktop/CHUNKS/PROJECT/Chunks-LMS/web/src/modules/catalog) | Test packages, versions, sections, test items, CVR physics calculation ($TC \times LC \times TL$), CCI profiles/curves, and AI live test generator. | Firestore vocab / input prompts → `test_packages`, `test_package_versions`, `test_sections`, `test_items`, `cci_profiles` → feeds Scheduling and Assessment. |
...
| [`lib/test-packages`](file:///C:/Users/gensh/Desktop/CHUNKS/PROJECT/Chunks-LMS/web/src/lib/test-packages.ts) | Data access layer & API client for test packages, sections, items, CCI categories, and narration generation. | UI / Studio → Supabase RPC & client queries, Edge functions (`generate-narration`, `get-playback-url`) → `narration_variants`. |
