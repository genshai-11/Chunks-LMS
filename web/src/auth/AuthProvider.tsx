      ...
      {authMethod === 'magic' ? (
        ...
        <div className="flex flex-col gap-4">
          <form
            ...
            onSubmit={(e) => {
              ...
              void action
                .then((result) => {
                  if (result.ok) {
                    setMessage(isSignUp ? (result as any).message || 'Account created successfully!' : 'Signed in successfully!')
                  } else {
                    setError(result.error)
                  }
                })
          ...
          >
            ...
            >
              <LogIn className="h-4 w-4" aria-hidden />
              <span>{submitting ? 'Please wait…' : isSignUp ? 'Create Account' : 'Sign in with Password'}</span>
            </button>
          </form>
