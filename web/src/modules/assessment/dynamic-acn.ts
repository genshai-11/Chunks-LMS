...
export function calculateDynamicAcn(
  ...
  if (preset === 'v2_completed') {
    ...
    acnTitle = `ACN = (N_total - Tổng n(Green + Blue + Indigo)) / Completed = (${nTotal} - ${totalN}) / ${finalizedCount} = ${acn.toFixed(2)}`
    ...
    acnTitle = `ACN = (N_total - Tổng n(Green + Blue + Indigo)) / Fixed = (${nTotal} - ${totalN}) / ${totalItemsCount} = ${acn.toFixed(2)}`
    ...
    acnTitle = `ACN = Average Chunks Number among ${probedCount} probed finalized question${probedCount === 1 ? '' : 's'}. Green opens at 1; each Continue adds 1.`
    ...
    try {
      ...
      acnTitle = 'Error evaluating custom formula'
    }
  } else {
    // Default fallback
    denominator = Math.max(finalizedCount, 1)
