...
| G4 | **Sessions tab – set/override CCI+CVR mapping** | Measurement snapshot column is read-only. Admin can *see* `activeCciLabel / activeTargetCvrOhm` but cannot set or override the mapping from this tab. Must navigate to Live Tests page, which is opaque. |
